# Base-EC2-GPU Infrastructure Module - Variables
...
variable "compute_profile" {
  type = object({
    ...
    gpu_type  = optional(string, "nvidia-t4")
  })
  description = "Compute resources requested for the workspace"

  validation {
    condition     = var.compute_profile.cpu >= 1 && var.compute_profile.cpu <= 96
    error_message = "CPU count must be between 1 and 96."
  }

  validation {
    condition     = var.compute_profile.gpu_count >= 1 && var.compute_profile.gpu_count <= 8
...
variable "image_id" {
  type        = string
  description = "AMI ID for the workspace (leave empty to use Deep Learning AMI)"
  default     = ""
}
...
variable "capabilities" {
  type = object({
    ...
    gpu_support       = optional(bool, true)
  ...
  validation {
    ...
    error_message = "GPU support must be enabled for GPU workspaces."
  }

  validation {
    condition     = !var.capabilities.gui_rdp
    error_message = "RDP is not supported on Linux GPU workspaces."
  }
}
...
variable "os_type" {
  ...
  validation {
    condition     = contains(["amazon-linux-2023", "ubuntu-22.04"], var.os_type)
    error_message = "OS type must be one of: amazon-linux-2023, ubuntu-22.04."
  }
}

variable "ami_owners" {
  type        = list(string)
  description = "AMI owner account IDs for Deep Learning AMI lookup"
  default     = ["amazon"]
}
