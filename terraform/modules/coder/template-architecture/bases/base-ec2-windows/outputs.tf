# Base-EC2-Windows Infrastructure Module - Outputs
...
output "ebs_volume_id" {
  description = "EBS volume ID for user data (if persistent_home enabled)"
  value       = var.capabilities.persistent_home ? aws_ebs_volume.data[0].id : null
}

output "remote_desktop_type" {
  description = "Remote desktop protocol in use"
  value       = var.remote_desktop_type
}
